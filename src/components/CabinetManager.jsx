...
export default function CabinetManager({ cabinets, lamps, reloadData }) {
    ...
    // Hàm hiển thị trạng thái dựa trên status và isOn từ Backend
    const renderStatus = (cab) => {
        if (cab.status === 'Offline') return <span className="text-gray-500">🔌 Mất kết nối (Offline)</span>;
        if (cab.status === 'Online' && cab.isOn) return <span className="text-green-600">⚡ Đang hoạt động (Online)</span>;
        return <span className="text-red-500">🔌 Đã Ngắt</span>;
    };
